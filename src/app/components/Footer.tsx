...
export default function Footer(props: {
  repo?: string;
  btnReturn?: boolean;
  more?: { title: string; description: string | string[]; splitChar?: boolean };
}) {
  const funcoes: {
    repo: string | null;
    btnReturn: boolean;
    more?: { title: string; description: string | string[] } | null;
    splitChar?: boolean;
  } = {
    repo: props.repo || "https://github.com/KaneeSell/Funcoes-e-Modularizacao",
  ...
  return (
    ...
    >
      ...
      >
        <span className="w-full lg:w-1/3 lg:pe-10 justify-center flex flex-col lg:flex-row-reverse px-3 lg:px-0 gap-2 items-center md:items-start lg:items-center lg:justify-between">
          <figure className="flex w-full lg:absolute justify-center">
            <Image
              alt="Logo Kanee Sell"
              src={logo}
              width={50}
          ...
          {funcoes.repo && funcoes.more && (
            <span className="flex flex-col w-full items-center justify-center gap-2">
              {funcoes.repo && (
                <a
              ...
              {funcoes.more && (
                <Fragment>
                  <p className="text-sm font-semibold">{funcoes.more?.title}</p>
                  {typeof funcoes.more.description === "string" && (
                    <p className="text-sm">
                      {!funcoes.splitChar && funcoes.more.description}
                      {funcoes.splitChar &&
                        funcoes.more.description.split(", ").join(" • ")}
                    </p>
                  )}
                  <ol className="list-disc">
                    {funcoes.more.description instanceof Array &&
                      funcoes.more.description.map((description, index) => (
        ...
        <div className="w-full lg:w-1/3  justify-center flex flex-col px-3 gap-2 items-center md:items-end lg:items-center">
          ...
          <span className="flex flex-col text-base sm:gap-1 sm:text-sm text-nowrap items-center w-1/2 justify-center leading-3">
            <p className="text-base sm:text-sm font-semibold">Deploy</p>
            <p>Vercel</p>
          </span>
